...
class mesh_interp(bpy.types.Operator):
    """Tooltip"""
    ...
    curve_type : bpy.props.EnumProperty(
        items=[
            ('BSPLINE', 'B-Spline', "Set interpolation type to B-Spline", 1),
            ('NURBS', 'NURBS', "Set interpolation type to NURBS", 2)],
        name="Curve Type",
...
def register():
    bpy.utils.register_class(mesh_interp)
def unregister():
    bpy.utils.unregister_class(mesh_interp)
